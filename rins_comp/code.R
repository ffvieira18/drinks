...
directory <- "C:/Users/ffvie/PycharmProjects/Teste/Ring_Files/edges/"
...
setwd("C:/Users/ffvie/PycharmProjects/Teste/Result_Script/")
for(i in seq.int(1,length(files))){
  write.table(x = lista[[i]], file = paste0("ResultadoNos_", files[i], ".txt"), row.names = F, quote = F, sep = "\t")
}
...
for(i in seq.int(1,length(files))){
  write.table(x = lista[[i]], file = paste0("ResultadoGrafo_", files[i], ".txt"), row.names = F, quote = F, sep = "\t")
}
